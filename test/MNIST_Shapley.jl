...
# ╔═╡ a52cdc0a-daf5-11ed-2103-919e9e783def
begin
	using Flux
	using BSON: @load
	using MLDatasets
...
# ╔═╡ 7f054c6a-c6bc-427d-97e2-001f88607d2e
begin
	# using Pkg
	# Pkg.activate("..")
	# using JuliaShap
...
# ╔═╡ 17bf8110-7dca-4097-851e-5fc4a8bc5624
begin
	@load "./models/model_MNIST.bson" model
	train_x_raw, train_y_raw = MNIST(split = :train)[:]
	test_x_raw,  test_y_raw  = MNIST(split = :test)[:]
end;

# ╔═╡ 59b2ca7c-f1b9-4ceb-8163-0869d1fb580c
model

# ╔═╡ 21b9e68f-3094-4be4-a4cc-91bbebe47368
f(x) = model(x[:])

# ╔═╡ fa24629e-0eee-4d2d-aeae-bb36d13c8618
five = train_x_raw[:, :, 1];

# ╔═╡ c412bc19-a091-47b1-a235-8bcdfa097626
prediction = argmax(model(five[:]))

# ╔═╡ ee6dfc44-24d3-4d5f-9c69-cfe1cb10a340
f(five)

# ╔═╡ 793bb79c-cd8f-4ca5-ba6d-4a970074c453
begin
	background = Vector{Matrix{Float32}}()
	for i ∈ 1:2000
		push!(background, train_x_raw[:, :, i])
	end
end

# ╔═╡ 8f77381b-79f9-4e3e-9491-fdde4072aac0
σ = reshape(randperm(28 * 28), (28, 28))

# ╔═╡ cde4fc5a-6fba-4055-aaa0-0e590bd7ba92
background[rand(1:length(background))][σ]

# ╔═╡ 850866e8-8d97-4555-a333-0f3cbd205dc9
function shapley_values_monte(f, x, background, n)
    shapley_values = zeros((size(x)..., 10))
    number_of_entries = foldl(*, size(x))
    size_of_background = length(background)
    n_rows = size(x)[1]
    n_cols = size(x)[2]
    for _ ∈ 1:n
        σ = reshape(randperm(number_of_entries), size(x))
        xₒ  = x#[σ]
        zₒ  = background[rand(1:size_of_background)][σ]
        x₊ⱼ = deepcopy(zₒ)
        x₋ⱼ = deepcopy(zₒ)
...
# ╔═╡ 8bdeef49-8b12-4426-8848-53e19489e51e
function expectation(f, k, background)
	sample_size = length(background)
	image_size = size(background[1])
    expectation = zeros(image_size)
	for i ∈ 1:foldl(*, image_size)
		predictions = Vector{Float32}(undef, sample_size)
		Threads.@threads for j ∈ 1:length(background)
			sample = background[j]
			original = sample[i]
			sample[i] = 0  # todo: set to "0-color" in final version
			predictions[j] = f(sample)[k]
			sample[i] = original
		end
		expectation[i] = sum(predictions) / sample_size
	end
	return expectation
end

# ╔═╡ a0f029d4-8148-4c27-b834-e2345f21bb3c
function expectation2(f, background)
	sample_size = length(background)
	image_size = size(background[1])
	expectation = zeros(image_size..., 10)
    number_of_entries = foldl(*, image_size)
    n_cols = image_size[2]
	x₊ⱼ = zeros(Float32, image_size)
	for i in 1:number_of_entries
		σ = reshape(randperm(number_of_entries), image_size)
		predictions = Vector{Vector{Float32}}()
		row = div(i - 1, n_cols) + 1
		col = rem(i - 1, n_cols) + 1
    	for j ∈ 1:sample_size
        	xₒ = background[j][σ]
            x₊ⱼ[i] = xₒ[i]
            push!(predictions, f(x₊ⱼ))
        end
		expectation[row, col, :] = sum(predictions) ./ sample_size
    end
    return expectation
end

# ╔═╡ 9124219c-f159-459a-9e3b-79fa82f92fe7
function sv_ref(f, x, E)
	sv = zeros(size(x)..., 10)
	n_cols = size(x)[2]
	for i ∈ 1:length(x)
		original = x[i]
		x[i] = 0  # todo: see expectation
		row = div(i - 1, n_cols) + 1
		col = rem(i - 1, n_cols) + 1
		sv[row, col, :] += f(x) - E[row, col, :]
		x[i] = original
	end
	sv# ./= length(x)
end

# ╔═╡ decb276f-6efb-4593-a872-d7405a8a7e13
ex_five = shapley_values_monte(f, five, background, 100);

# ╔═╡ f346a181-4a9a-4a1a-a0a5-719f1f676777
maximum(ex_five), minimum(ex_five)

# ╔═╡ 88df18a3-c63c-48a8-9f9c-9c39023ed1f2
size(ex_five)

# ╔═╡ 4f257b29-835d-4dd9-94a5-e2a634a05c02
image_to_display = ex_five;

# ╔═╡ 7095fbb9-dce7-437a-b884-1974a4236672
begin
	figure, axes = plt.subplots(1, 10, figsize=(20, 20))
	maxabs1 = abs(maximum(image_to_display))
	minabs1 = abs(minimum(image_to_display))
	vmax1 = maximum([maxabs1, minabs1])
	for i ∈ 1:10
		axes[i].imshow(image_to_display[:, :, i], cmap="seismic", vmin=-vmax1, vmax=vmax1)
	end
	
end

# ╔═╡ f1cb9a0b-1274-47d1-868d-a033958635cc
figure

# ╔═╡ 5531eeef-4f07-4d7c-8c59-693f4dd979b1
n_samples = 100

# ╔═╡ 34d6a3db-2623-4fcf-9e2a-82faa9e06a55
𝔼 = expectation2(f, background[2:n_samples + 1]);

# ╔═╡ f98f67ab-17d4-4f6f-a1e4-5343251933ec
typeof(𝔼)

# ╔═╡ 968e3d70-2150-4473-b7b1-3ffe1e353fb3
SV = sv_ref(f, five, 𝔼);

# ╔═╡ 9d43083e-a00f-4628-b8ae-9eb07e86b2f5
maximum(SV[:, :, 6]), minimum(SV[:, :, 6])

# ╔═╡ db4f5f36-b5de-407e-8919-7140e021a725
begin
	figure2, axes2 = plt.subplots(1, 11, figsize=(20, 20))
	maxabs = abs(maximum(SV[:, :, 6]))
	minabs = abs(minimum(SV[:, :, 6]))
	vmax = maximum([maxabs, minabs])
	
	axes2[1].imshow(five', cmap="gray")
	for i ∈ 2:11
		axes2[i].imshow(SV[:, :, i - 1], cmap="seismic", vmin=-vmax, vmax=vmax)
	end
	im = axes2[6].imshow(SV[:, :, 6], cmap="seismic", vmin=-vmax, vmax=vmax)
	figure2.colorbar(im, ax=axes2, orientation="horizontal")
end

# ╔═╡ bcb0a1ae-5c25-4cee-a454-e48b18ee9336
figure2

# ╔═╡ 504f2520-6d03-4dd5-ae4d-a026a55a9bce
function normalize(M)
	maxabs = abs(maximum(M))
	minabs = abs(minimum(M))
	copy(M) ./= maximum([maxabs, minabs])
end
...
julia_version = "1.8.5"
manifest_format = "2.0"
project_hash = "283abe16bc47abad12c12eb97a8ed6dab7eb5eac"
