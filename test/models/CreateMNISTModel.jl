...
# ╔═╡ bf628792-dacf-11ed-1fe4-f59a24a7d118
begin
	using Flux
	using MLDatasets
	using ImageIO
...
# ╔═╡ d41e94af-195e-4db6-b873-58db1dee3b7e
using BSON: @save

# ╔═╡ a8048bbd-bf3a-4e59-bdea-0a3f1612087d
md"""
Based on [Towards Datascience - Flux.jl on MNIST — Variations of a theme](https://towardsdatascience.com/flux-jl-on-mnist-variations-of-a-theme-c3cd7a949f8c)
"""

# ╔═╡ bb86ba2c-f67b-442c-8aeb-5437480464dc
mnist = MNIST()

# ╔═╡ 3175a080-9cb4-4d0e-94b7-bde4f7f4d62c
train_x_raw, train_y_raw = MNIST(split = :train)[:];

# ╔═╡ 0032a0cc-8f80-4815-8994-f503f10cffc1
test_x_raw,  test_y_raw  = MNIST(split = :test)[:];

# ╔═╡ dc6a00f3-4e1c-475a-a9eb-6fab9d7a1a93
convert2image(MNIST, train_x_raw[:,:,1])

# ╔═╡ 6c9b62ca-6d07-4ff1-a635-570de38edbbc
train_x = Flux.flatten(train_x_raw);

# ╔═╡ 71ba067a-dc15-4c79-a539-f962f188c4ad
test_x  = Flux.flatten(test_x_raw);

# ╔═╡ 39357120-a8e4-4404-9914-f5ad9f9a421e
train_y = Flux.onehotbatch(train_y_raw, 0:9);

# ╔═╡ e2edc94b-226d-4e28-a1bd-22aae27b08d3
test_y  = Flux.onehotbatch(test_y_raw, 0:9);

# ╔═╡ cb25ddc5-2643-43cc-a13f-1c8eb4b60e45
model4LS = Chain(
...
# ╔═╡ 36b737a2-8c7f-4185-afea-c96d53d7ad24
model3LS = Chain(
...
# ╔═╡ 74f43092-fb62-47b0-a5e8-a6ba742d00ca
model2LR = Chain(
...
# ╔═╡ dfdc3d9d-4201-46d9-960d-6c66f036648e
begin
...
# ╔═╡ 1eece0b0-5e2a-46c7-bec2-75eca238d8da
begin
...
# ╔═╡ 35609978-9665-4086-890b-db02156c03dc
...
# ╔═╡ c6c7e390-425b-44c2-a159-d22e6989c321
function train_stochastic(X, y, loss, opt, params, epochs)
    m = size(X)[2]          # number of instances
    for epoch in 1:epochs  
        rindex = rand(1:m)                          # choose a random index within the training dataset
        data = [(X[:, rindex], y[:, rindex])]       # extract the instance at that index position 
        Flux.train!(loss, params, data, opt)
    end
end

# ╔═╡ d8b9cf24-012c-4109-8576-dd077e7ee8ac
function train_minibatch(X, y, loss, opt, params, epochs, batchsize = 100; logstep = 100)
    hist = zeros(Float64, epochs ÷ logstep)             # initialize array for loss-history
    m = size(X)[2] - batchsize + 1
    for epoch in 1:epochs
        rindex = rand(1:m)	
        data = [(X[:, rindex:rindex+batchsize-1], 
		 y[:, rindex:rindex+batchsize-1])]
        Flux.train!(loss, params, data, opt)
        if epoch % logstep == 0                         
            hist[epoch ÷ logstep] = loss(X, y)          # store current loss in loss-history
        end
    end
    return(hist)                                        # return loss-history
end

# ╔═╡ e987d4a2-1663-4f25-b51d-1b1f9716ae18
train_minibatch(train_x, train_y, loss2LRce, 
            Descent(0.2), params2LR, 1000)

# ╔═╡ 54b67819-b70d-495b-82da-c82b0091112d
loss2LRce(train_x, train_y)

# ╔═╡ 93e536cb-6fcb-4d9a-a8b0-7d92819b8974
model = model2LR

# ╔═╡ c10498c6-fb39-4cea-a5fe-a7afdfd23c9f
k = 4

# ╔═╡ 3098ed62-31df-4b92-b339-504896b322d4
model(test_x)[:, k]

# ╔═╡ 61f6aee7-ab15-4c47-b474-20557567a945
argmax(model(test_x)[:, k])

# ╔═╡ 542a3d09-2987-4d8c-a3ca-887bb0ead219
convert2image(MNIST, test_x_raw[:, :, k])

# ╔═╡ 8590351a-291c-4c37-8e66-52b40fb144af
function accuracy(y_hat, y_raw)
    y_hat_raw = Flux.onecold(y_hat) .- 1
    count(y_hat_raw .== y_raw) / length(y_raw)
end

# ╔═╡ 46df73e5-b3b8-49b6-ae47-8f626656dcfd
accuracy(model(test_x), test_y_raw)

# ╔═╡ 31c8c8e3-105e-4030-b9fe-006835220502
@save "model_MNIST.bson" model
...
julia_version = "1.8.5"
manifest_format = "2.0"
project_hash = "15b3727e0d3f303d34f8c11f682485e6beaa2fbb"
